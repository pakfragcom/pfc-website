...
export default function FragrancesIndex({ fragrances = [] }) {
  ...
  useEffect(() => {
    ...
    setActiveCategory(CATEGORIES.find(c => c.id === cat) ? cat : 'all');
    setQuery(q);
    setSortBy(SORT_OPTIONS.find(s => s.id === sort) ? sort : 'name');
  ...
  const categoryCounts = useMemo(() => {
    const counts = {};
    fragrances.forEach(f => { counts[f.category] = (counts[f.category] || 0) + 1; });
    return counts;
  }, [fragrances]);
            ...
            <div className="mx-auto max-w-6xl px-6">
              ...
              <div className="flex flex-col gap-3 mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map(cat => {
                    const count = cat.id === 'all' ? fragrances.length : (categoryCounts[cat.id] || 0);
                    return (
                      <button key={cat.id} onClick={() => { setActiveCategory(cat.id); updateFilter(cat.id, query, sortBy); }}
...
function FragranceCard({ f }) {
  ...
  const reviewUrl = `/reviews/submit?fragrance=${encodeURIComponent(f.name)}&house=${encodeURIComponent(f.house)}&fid=${f.id}`;

  return (
    <div className="group rounded-2xl border border-white/10 bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.05] transition-all duration-300">
      <Link href={`/fragrances/${f.slug}`} className="block">
        {/* Image / placeholder */}
...
export async function getStaticProps() {
  ...
  const fragrances = (data || []).map(f => {
    ...
    return {
      ...
      slug: f.slug,
      house: f.house,
      house_slug: f.fragrance_houses?.slug || null,
      category: f.category,
